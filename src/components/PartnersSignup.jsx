...
const PatnersSignup = () => {
    ...
    const getPan = async (data) => {
        try {
            ...
            if (!response.data.success) {
                const errMsg = response.data.message || "PAN verification failed. Please check your PAN and Date of Birth and try again.";
                console.error("⚠️ PAN verification failed:", errMsg);
                alert("❌ " + errMsg);
                setKycExist("False");
                setKycStatus(response.data.panStatus || null);
                return; // ← UI stays on this step
            }

            // ✅ Valid KYC code — advance UI
            ...
            console.error("PAN check request failed:", error);
            const serverMsg = error.response?.data?.message;
            alert("❌ " + (serverMsg || "PAN verification request failed. Please try again."));
        } finally {
    ...
    const handleDownloadPdf = async () => {
        ...
        try {
            ...
            if (identifier) {
                await api.post("/api/partners/confirm", {
                    identifier: identifier,
                    entityName: watchReg("entityName") || "Partner",
                    address: watchReg("address") || "",
                    arn: watchRegulatory("arn") || "",
                    aprn: watchRegulatory("aprn") || "NA",
                });
                alert("Registration confirmed! Your agreement has been sent to your registered email address.");
                navigate("/");
            }
        } catch (error) {
            console.error("Agreement processing failed:", error);
                        ...
                        >
                            ...
                            <div className="mb-8 text-justify" style={{ fontFamily: PoppinsRegular }}>
                                ...
                                <p className="mb-8 font-bold">
                                    {entityName.toUpperCase()}, <span className="font-normal">having its principal place of business at</span> {address} <span className="font-normal">and engaged in the business of financial product marketing and client relationship management (hereinafter referred to as the "Sub-Distributor", which expression shall unless repugnant to the context include its successors and permitted assigns). The sub-distributor holds ARN –</span> {arn} <span className="font-normal">and APRN -</span> {aprn}
                                </p>
                                <p className="mb-10">
